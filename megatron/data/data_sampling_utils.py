...
from megatron.data.agent import SACWeightUpdater
...
class WeightedRandomSampler(torch.utils.data.Sampler):
    ...
    def _reset(self, generator):
        self._samplers = [iter(torch.utils.data.RandomSampler(dataset, generator=generator)) for dataset in
                          self.datasets.values()]

    def _reset_single(self, dataset_name: str, generator):
        self._samplers[self._inverse_dataset_map[dataset_name]] = iter(
            torch.utils.data.RandomSampler(self.datasets[dataset_name], generator=generator))
...
class EMAWeightUpdater:
    ...
    def update(self, dataset_name: str, reward: float, iteration: int) -> List[float]:
        self.weights[dataset_name] = (self.smoothing_factor / (1 + iteration)) * reward + (
                    1 - self.smoothing_factor / (1 + iteration)) * self.weights[dataset_name]
...
class Exp3WeightUpdater:
    ...
    def update(self, dataset_name: str, reward: float, iteration: int) -> List[float]:
        ...
        for name in self.dataset_names:
            self.weights[self.dataset_map[name]] = math.exp(
                self._cumulative_estimated_reward[name] * self.prev_eps) * scaling_factor + self.eps
...
class SmoothedMeanWeightUpdater:
    ...
    def update(self, dataset_name: str, reward: float, iteration: int) -> List[float]:
        ...
        # update cumulative estimated reward
        self._estimated_reward[dataset_name] = self.smoothing_factor * self._estimated_reward[dataset_name] + (
        ...
        for name in self.dataset_names:
            self.weights[self.dataset_map[name]] = math.exp(
                self._estimated_reward[name] * self.prev_eps) * scaling_factor + self.eps
    ...
    def group_update(self, dataset_names: List[str], rewards: List, iteration: int):
        ...
        for name, reward in zip(dataset_names, rewards):
            ...
            self._estimated_reward[name] = self.smoothing_factor * self._estimated_reward[name] + (
                        1 - self.smoothing_factor) * math.exp(reward)
        ...
        for name in self.dataset_names:
            ...
            self.weights[self.dataset_map[name]] = self._estimated_reward[
                                                       name] * self.prev_eps * scaling_factor + self.eps
...
def get_weight_updater(update_method: str, dataset_names, weights, **kwargs):
    ...
    # hds
    elif update_method == "sac":
        return SACWeightUpdater(dataset_names=dataset_names, weights=weights)
...
def get_data_sampling_weighter(
    ...
    """
    print_rank_0(
        f"Creating data sampling weighter with:\n\tdataset_names: {dataset_names}\n\tweights: {weights}\n\twarmup_steps: {warmup_steps}\n\tupdate_frequency: {update_frequency}\n\tupdate_method: {update_method}\n\tkwargs: {kwargs}")
    if update_method is None:
        assert (
                    warmup_steps == 0 and update_frequency == 0), "Must provide update method if warmup steps or update frequency are provided."
