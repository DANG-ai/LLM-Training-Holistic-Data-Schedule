    ...
    def __getitem__(self, idx):
        try:
            ...
            if self.name_passthrough:
                return {"text": torch.tensor(np.array(sample, dtype=np.int64), dtype=torch.int64), "dataset_name": self.name}
            else:
                return {"text": torch.tensor(np.array(sample, dtype=np.int64), dtype=torch.int64)}
        except IndexError:
